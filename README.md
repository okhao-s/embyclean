...
- **时长查重（duration）**
  - 支持对比范围切换：`同目录` / `同库`
  - 支持时长精度切换：`整秒` / `两位小数`
  - 默认按 `同目录 + 整秒` 分组，避免一上来把整库近似时长混到一起
  - `整秒`：按秒四舍五入归组，适合先粗扫
  - `两位小数`：按 0.01 秒四舍五入归组，适合精细排查
  - 在所选范围 + 所选精度内落在同一时长键的资源视为重复
  - 适合同目录下不同文件名但内容近似相同的资源；切到 `同库` 或更高精度时可扩大/细化排查面
- **缺失封面（noposter）**
  - 列出没有 Primary 海报的项目
- **极小文件（tiny）**
  - 找异常小文件，也可叠加时长阈值
...
- `GET /api/scan`
  - 执行扫描并返回序列化结果
  - `duration` 模式额外支持 `duration_scope=dir|library`，默认 `dir`
  - `duration` 模式额外支持 `duration_precision=second|centisecond`，默认 `second`
- `GET /api/tasks`
- `POST /api/tasks`
- `PUT /api/tasks/{id}`
- `DELETE /api/tasks/{id}`
...
- 其中 `duration` 模式的“忽略”是**按当前分组**生效：分组签名基于「范围（同目录/同库） + 范围桶 key + 时长精度（整秒/两位小数） + 当前精度下的时长键 + 当前成员 Emby ID 集合」生成
- 所以它只会隐藏本次命中的这个时长分组；同目录/同库、整秒/两位小数彼此独立，不会互相串到同一条忽略记录
- 这也意味着切换范围或切换精度后，之前在另一种口径下忽略过的分组不会被误复用

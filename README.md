...
- **时长查重（duration）**
  - 支持对比范围切换：`同目录` / `同库`
  - 默认按 `同目录` 分组，避免一上来把整库近似时长混到一起
  - 两种范围都按 1 位小数精度分组（四舍五入到 0.1 秒）
  - 在所选范围内落在同一 0.1 秒分组的资源视为重复
  - 适合同目录下不同文件名但内容近似相同的资源；切到 `同库` 时可扩大排查面
- **缺失封面（noposter）**
  - 列出没有 Primary 海报的项目
- **极小文件（tiny）**
  - 找异常小文件，也可叠加时长阈值
...
- 其中 `duration` 模式的“忽略”是**按当前分组**生效：分组签名基于「范围（同目录/同库） + 范围桶 key + 0.1 秒时长键 + 当前成员 Emby ID 集合」生成
- 所以它只会隐藏本次命中的这个时长分组；同目录和同库各自独立，不会互相串到同一条忽略记录
- 这也意味着切换范围后，之前在另一种范围下忽略过的分组不会被误复用
